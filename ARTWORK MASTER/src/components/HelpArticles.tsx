  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-beith-gray-900">Help & Correction Guide</h2>
        {onClose && (
          <button
            onClick={onClose}
            aria-label="Close"
            className="text-beith-gray-500 hover:text-beith-gray-700"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {filteredArticles.map(article => (
              ...
              >
                ...
                <div className="text-sm text-beith-gray-600">{article.category}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          {currentArticle ? (
            <div>
              <h3 className="text-xl font-semibold text-beith-gray-900 mb-2">
                {currentArticle.title}
              </h3>
              <div className="text-sm text-beith-gray-600 mb-4">
                Category: {currentArticle.category}
              </div>
              
              <p className="text-beith-gray-700 mb-6">{currentArticle.content}</p>
              
              {currentArticle.steps && (
                <div>
                  <h4 className="font-semibold text-beith-gray-900 mb-3">Step-by-Step Instructions:</h4>
                  <ol className="space-y-3">
                    {currentArticle.steps.map((step, index) => (
                      <li key={index} className="flex items-start space-x-3">
                        ...
                        <span className="text-beith-gray-700">{step}</span>
                      </li>
                    ))}
                  </ol>
                </div>
              ...
              {currentArticle.relatedRules && (
                <div className="mt-6 pt-4 border-t border-beith-gray-200">
                  <h4 className="font-semibold text-beith-gray-900 mb-2">Related Validation Rules:</h4>
                  <div className="flex flex-wrap gap-2">
                    {currentArticle.relatedRules.map(rule => (
                      <span
                        key={rule}
                        className="px-2 py-1 bg-beith-gray-100 text-beith-gray-700 rounded text-sm"
                      >
                        {rule}
                      </span>
                    ))}
            ...
            <div className="text-center text-beith-gray-500 py-12">
