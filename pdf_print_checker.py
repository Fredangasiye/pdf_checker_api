...
def extract_pdf_text_ocr(uploaded_pdf):
    full_text = ""
...
st.title("📄 Batch PDF Print Checker - Mac App Ready")
...
if uploaded_pdfs:
    for uploaded_pdf in uploaded_pdfs:
        ...
        text_layer = extract_pdf_text(uploaded_pdf)

        if not text_layer.strip():
            st.warning("No text layer detected, applying OCR...")
            uploaded_pdf.seek(0)
            full_text = extract_pdf_text_ocr(uploaded_pdf)
        else:
...
st.caption("This app is ready for Mac standalone packaging with py2app.")
